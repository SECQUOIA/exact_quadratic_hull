...
def build_constrained_layout_model(
    ...
    assert len(rect_lengths) == len(rect_heights), (
        "There should be the same number of rectangle lengths and heights."
    )
    assert len(circ_xvals) == len(circ_yvals) == len(circ_rvals), (
    ...
    m.circle_x = Param(m.circles, initialize=circ_xvals, doc="x-coordinate of circle center")
    m.circle_y = Param(m.circles, initialize=circ_yvals, doc="y-coordinate of circle center")
    ...
    def x_min(m, rect):
        return min(
            m.circle_x[circ] - m.circle_r[circ] + m.rect_length[rect] / 2 for circ in m.circles
        )
    ...
    def y_min(m, rect):
        return min(
            m.circle_y[circ] - m.circle_r[circ] + m.rect_height[rect] / 2 for circ in m.circles
        )
    ...
    m.rect_pairs = Set(initialize=m.rectangles * m.rectangles, filter=lambda _, r1, r2: r1 < r2)
    ...
    m.rect_x = Var(m.rectangles, doc="x-coordinate of rectangle center", bounds=x_bounds)
    m.rect_y = Var(m.rectangles, doc="y-coordinate of rectangle center", bounds=y_bounds)
    ...
    @m.Disjunction(
        m.rect_pairs,
        doc="Make sure that none of the rectangles overlap in either the x or y dimensions.",
    )
    def no_overlap(m, r1, r2):
        return [
            m.rect_x[r1] + m.rect_length[r1] / 2 <= (m.rect_x[r2] - m.rect_length[r2] / 2),
            m.rect_y[r1] + m.rect_height[r1] / 2 <= (m.rect_y[r2] - m.rect_height[r2] / 2),
            m.rect_x[r2] + m.rect_length[r2] / 2 <= (m.rect_x[r1] - m.rect_length[r1] / 2),
            m.rect_y[r2] + m.rect_height[r2] / 2 <= (m.rect_y[r1] - m.rect_height[r1] / 2),
        ]
...
if __name__ == "__main__":
    ...
    for d in ["l1", "l2"]:
        for key in constrained_layout_model_examples.keys():
            ...
            model = build_constrained_layout_model(constrained_layout_model_examples[key], metric=d)
