...
def _get_hull_eps_dash_style(index: int) -> tuple:
    """Return a distinct dash pattern for hull_eps variants based on index."""
    hull_eps_dash_styles = [
        (0, (12, 4)),
        (0, (8, 3)),
        (0, (4, 2)),
        (0, (2, 1)),
    ...
    return hull_eps_dash_styles[index % len(hull_eps_dash_styles)]
...
def _get_strategy_style_maps() -> tuple:
    ...
    convex_style_overrides = {
        "gdp.convex_flag_bigm": (0, (8, 3, 2, 3)),
        "gdp.convex_flag_hull_exact_conic_no_cholesky": (0, (6, 2, 2, 2)),  # distinct from Conic Exact Hull (3,1,1,1)
    }
    for convex_key, base_key in [
        ...
        style_map[convex_key] = convex_style_overrides.get(convex_key, style_map[base_key])
    color_map.update(convex_color_map)

    return style_map, color_map


def _populate_hull_eps_styles(strategies: list, style_map: dict, color_map: dict) -> None:
    """Assign distinct dash styles and colors to hull_eps_* strategies found in data."""
    hull_eps_colors = [
        "magenta", "red", "deepskyblue", "limegreen", "darkorange",
        "mediumpurple", "teal", "gold", "tomato", "slateblue",
    ]
    hull_eps_strategies = sorted(
        s for s in strategies if re.match(r"gdp\.(convex_flag_)?hull_eps_", s)
    )
    for i, strategy in enumerate(hull_eps_strategies):
        if strategy not in style_map:
            style_map[strategy] = _get_hull_eps_dash_style(i)
        if strategy not in color_map:
            color_map[strategy] = hull_eps_colors[i % len(hull_eps_colors)]
...
_INF_GAP_THRESHOLD = 1e10


def _is_finite_gap(series: pd.Series) -> pd.Series:
    """Return a boolean mask that is True for finite, non-infinity-like gap values.

    Treats NaN, inf, -inf, and values with absolute magnitude >= _INF_GAP_THRESHOLD
    as "infinity" and returns False for them.
    """
    return pd.notna(series) & np.isfinite(series) & (np.abs(series) < _INF_GAP_THRESHOLD)
...
def create_relaxation_gap_comparison(
    ...
    # Filter out rows where either gap is None/NaN/inf/very-large
    valid_data = merged[
        _is_finite_gap(merged[f"{col_name}_1"]) & _is_finite_gap(merged[f"{col_name}_2"])
    ]
...
def create_node_relaxation_comparison(
    ...
    # Filter out rows where either value is None/NaN/inf/very-large
    valid_data = merged[
        _is_finite_gap(merged[f"{col_name}_1"]) & _is_finite_gap(merged[f"{col_name}_2"])
    ]
...
def create_root_relaxation_scatter(
    ...
    # Filter out rows where either root relaxation value is NaN/inf/very-large
    valid_data = merged[
        _is_finite_gap(merged[f"{col_name}_x"]) & _is_finite_gap(merged[f"{col_name}_y"])
    ]
...
def create_dolan_more_performance_profile(
    ...
    _populate_hull_eps_styles(strategies, style_map, color_map)
...
def create_performance_profile(
    ...
    def create_gap_performance_profile(
        ...
        # Get all gap values that are finite and not infinity-like
        all_gaps = df_gap["Bound Absolute Gap"]
        finite_mask = _is_finite_gap(all_gaps)
        finite_gaps = all_gaps[finite_mask]
        ...
        for strategy in strategies:
            ...
            gap_data = strategy_data["Bound Absolute Gap"].copy()
            gap_data = gap_data[_is_finite_gap(gap_data)]
    ...
    def create_fraction_performance_profile(
        ...
        all_gaps = df["Bound Absolute Gap"]
        finite_gaps = all_gaps[_is_finite_gap(all_gaps)]
        if len(finite_gaps) == 0:
        ...
        for strategy in strategies:
            ...
            gap_data = strategy_data.loc[timeout_mask, "Bound Absolute Gap"].copy()
            gap_data = gap_data[_is_finite_gap(gap_data)]
            gap_data = np.sort(gap_data)
