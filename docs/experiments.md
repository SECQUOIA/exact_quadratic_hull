...
Total jobs equal instances × strategies × solvers. Every job rebuilds its model and uses a private scratch directory with `keepfiles=False`. The runner detects timeouts from Gurobi, BARON, and SCIP log messages, with GAMS solver status as an additional signal when listing text is available. It also rejects a nominally optimal result at the time limit when the final gap is nonzero or unknown. Aborted, warning, and licensing statuses are solver errors, not verified solutions.
