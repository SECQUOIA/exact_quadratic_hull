...
Random-quadratic and k-means instances accept `replicate`, a positive integer that defaults to 1. It changes only the normalized parameter map, derived seed, and instance ID, providing independent repetitions of an otherwise identical grid cell. Random-quadratic instances also accept `objective_positive_definite`, a Boolean that defaults to the value of `ensure_positive_definite`. The former controls PSD shifting of the objective matrix; the latter continues to control the disjunct-constraint matrices. Defaults are resolved before identity derivation, so an omitted `replicate` matches `replicate=1`, and an omitted objective setting matches the corresponding explicit Boolean.
