"""CSTR superstructure benchmark.

This model builds a CSTR superstructure with a first-order autocatalytic A -> B reaction
and minimizes total reactor-series volume.
The optimal solution should yield NT reactors with a recycle before reactor NT.

Reference: Liñán et al. (2020), "Optimal design of superstructures for placing units
and streams with multiple and ordered available locations. Part I," Computers &
Chemical Engineering 137, 106794. https://doi.org/10.1016/j.compchemeng.2020.106794
"""
...
def build_model(NT: int = 5) -> pyo.ConcreteModel:
    ...
    m.quad_link = pyo.Constraint(
        m.N, rule=quad_link_rule, doc="Link Q[n]^2 to auxiliary variable t[n]"
    )
...
class CstrBenchmark:
    ...
    def solution(self, model):
        return {"volume": {str(index): pyo.value(model.c[index]) for index in model.N}}


BENCHMARK = CstrBenchmark()
